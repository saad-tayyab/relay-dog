...
import { db } from '../db'
import { relays, relayInfoSnapshots, healthChecks } from '../db/schema'
...
relayRoutes.get('/', async (c) => {
  ...
  if (query.search) {
    conditions.push(sql`(${relays.name} ILIKE ${'%' + query.search + '%'} OR ${relays.url} ILIKE ${'%' + query.search + '%'})`)
  }
  if (query.country) {
  ...
  for (const row of results) {
    ...
    } else if (row.lastHealthCheck && (!existing.lastHealthCheck || row.lastHealthCheck.checkedAt > existing.lastHealthCheck.checkedAt)) {
...
relayRoutes.post('/', async (c) => {
  ...
  const [newRelay] = await db.insert(relays).values({
    url,
...
relayRoutes.post('/:id/check', async (c) => {
  ...
  const httpUrl = relay.url.replace(/^wss:\/\//, 'https://').replace(/^ws:\/\//, 'http://').replace(/\/$/, '')
  const wsUrl = relay.url.startsWith('http') ? relay.url.replace(/^https:\/\//, 'wss://').replace(/^http:\/\//, 'ws://') : relay.url
  ...
  // Store health check
  const [healthCheck] = await db.insert(healthChecks).values({
    relayId: id,
    httpReachable,
    corsConfigured,
