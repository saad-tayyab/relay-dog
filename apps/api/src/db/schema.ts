...
export const relayInfoSnapshots = pgTable('relay_info_snapshots', {
  id: uuid('id').primaryKey().defaultRandom(),
  relayId: uuid('relay_id').notNull().references(() => relays.id, { onDelete: 'cascade' }),
...
export const monitoringJobs = pgTable('monitoring_jobs', {
  id: uuid('id').primaryKey().defaultRandom(),
  relayId: uuid('relay_id').notNull().references(() => relays.id, { onDelete: 'cascade' }).unique(),
  ...
  index('monitoring_jobs_enabled_idx').on(table.enabled),
])
